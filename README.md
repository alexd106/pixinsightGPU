# GPU Software Installer for PixInsight (Ubuntu 24.04 + GeForce RTX 2060 GPU)
...
1) Check GPU & prerequisites
2) Install CUDA
3) Install cuDNN
4) Install TensorFlow C API
5) Normalize cuDNN symlinks (fix ldconfig warnings)
6) Install All
7) Verify installed components
8) Quit

**Option 1:** Installs missing system packages like `build-essential`, `wget`, `curl`, `binutils`, etc. Also checks if a compatible NVIDIA GPU is present on the system and if the correct NVIDIA driver is installed. It will also check if the Nouveau driver is currently being used. If the nouveau driver is active, the script will stop and prompt the user to disable it. Checks whether a working installation of PixInsight is present on the system. This Option is useful as a quick check prior to installing other (or all) components (although each component will check pre-requisites anyway). 
...
**Option 6:** Installs all components: CUDA, cuDNN and TensorFlow (see **Note 1** below).
